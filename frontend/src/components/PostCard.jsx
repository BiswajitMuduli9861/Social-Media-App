...
const PostCard = ({ post }) => {
    ...
    return (
        ...
        <div className='bg-white rounded-xl shadow p-4 space-y-4 w-full max-w-2xl '>
            ...
            {/* Content */}
            {post.content && <div className='text-gray-800 text-sm whitespace-pre-line ' dangerouslySetInnerHTML={{__html: postWithHashtags}}/>}  //postWithHashtags  |Work is #content color change|

            {/* Post Images */}
            <div className='gird grid-cols-2 gap-2'>
                {
                    post.image_urls.map((img, index)=>(
                        <img src={img} alt="img" key={index} className={`w-full h-100 object-cover rounded-lg ${post.image_urls/length === 1 && 'col-span-2 h-auto'}`}/>
                    ))
                }
            </div>
            {/* Actions Button */}
