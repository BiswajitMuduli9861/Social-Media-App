        ...
        <div className='fixed inset-0 z-110 min-h-screen bg-black/80 backdrop-blur text-white flex items-center justify-center p-4'>
            <div className="w-full max-w-md">
                ...
                <div className='flex mt-4 gap-2'>
                    {
                        bgColors.map((color) => (
                            <button key={color} onClick={() => setBackground(color)} style={{ backgroundColor: color }} className='w-6 h-6 rounded-full ring cursor-pointer' />
                        ))
                    }
                </div>

                <div className='flex gap-2 mt-4'>
                    <button className={`flex-1 flex items-center justify-center gap-2 p-2 rounded ${mode === 'text' ? 'bg-white text-black' : "bg-zinc-800"} cursor-pointer`}>
