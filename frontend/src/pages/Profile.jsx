          ...
          <div className='bg-white rounded-xl shadow p-1 flex max-w-md mx-auto'>
            {
              ['posts', 'media', 'likes'].map((tab)=>(
                <p key={tab} onClick={()=> setActiveTab(tab)} className={`flex-1 px-4 py-2 text-sm font-medium rounded-lg transition-colors cursor-pointer ${activeTab === tab ? "bg-indigo-600 text-white" : "text-gray-600 hover:text-gray-900"}`}>
  ...
  <div className='flex flex-col mt-6 items-center gap-6'>
    {posts.length > 0 ? (
      ...
      <p className="text-gray-500">No posts yet</p>
          ...
          {
            activeTab === 'media' &&(
              <div className='flex  mt-6 max-w-6xl'>
                {posts.filter((post) => post.image_urls.length > 0).map((post)=>(
                  <>
                    {post.image_urls.map((image, index)=>(
                      <Link target='_blank' to={image} key={index} className='relative group'>
                        <img src={image} key={index} alt="image" className='w-64 aspect-video object-cover'/>
